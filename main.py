...
class Dashboard:
    ...
    def _render_kpi_deep_dive_tab(self):
        ...
        """
        st.subheader("Key Risk Indicator Profiles")
        ...
        try:
            ...
            for i, row in df_top.iterrows():
                ...
                with col2:
                    ...
                    with sub_cols[0]:
                        ...
                        st.markdown(f"<p style='color: {colors['Violence']}; font-size: 14px; margin-bottom: -5px;'><b>Violence Risk</b></p>", unsafe_allow_html=True)
                        fig_v = go.Figure(go.Indicator(
                            mode = "gauge+number",
                            value = violence_score * 100,
                            number = {'font': {'size': 28}},
                            gauge = {'axis': {'range': [None, 100]}, 'bar': {'color': colors['Violence'], 'thickness': 1}}))
                        fig_v.update_layout(height=60, margin=dict(l=0,r=0,t=10,b=10), paper_bgcolor='rgba(0,0,0,0)')
                        st.plotly_chart(fig_v, use_container_width=True)

                    # Medical Score
                    with sub_cols[1]:
                        medical_score = row['Medical Surge Score']
                        st.markdown(f"<p style='color: {colors['Medical']}; font-size: 14px; margin-bottom: -5px;'><b>Medical Risk</b></p>", unsafe_allow_html=True)
                        fig_m = go.Figure(go.Indicator(
                            mode = "gauge+number",
                            value = medical_score * 100,
                            number = {'font': {'size': 28}},
                            gauge = {'axis': {'range': [None, 100]}, 'bar': {'color': colors['Medical'], 'thickness': 1}}))
                        fig_m.update_layout(height=60, margin=dict(l=0,r=0,t=10,b=10), paper_bgcolor='rgba(0,0,0,0)')
                        st.plotly_chart(fig_m, use_container_width=True)

                    # Spillover Score
                    with sub_cols[2]:
                        spillover_score = row['Spatial Spillover Risk']
                        st.markdown(f"<p style='color: {colors['Spillover']}; font-size: 14px; margin-bottom: -5px;'><b>Spillover Risk</b></p>", unsafe_allow_html=True)
                        fig_s = go.Figure(go.Indicator(
                            mode = "gauge+number",
                            value = spillover_score * 100,
                            number = {'font': {'size': 28}},
                            gauge = {'axis': {'range': [None, 100]}, 'bar': {'color': colors['Spillover'], 'thickness': 1}}))
                        fig_s.update_layout(height=60, margin=dict(l=0,r=0,t=10,b=10), paper_bgcolor='rgba(0,0,0,0)')
            ...
            st.divider()
            st.subheader("Advanced Analytical Visualizations")
            # ... (the rest of your tabbed visualizations) ...
