...
def cache_current_incidents(_dm: DataManager, env_factors_dict: dict) -> list:
    return _dm.get_current_incidents(EnvFactors(**env_factors_dict))

@st.cache_data(show_spinner=False)
def cache_kpis(_engine: PredictiveAnalyticsEngine, historical_data: list, env_factors_dict: dict, incidents: list) -> pd.DataFrame:
    return _engine.generate_kpis(historical_data, EnvFactors(**env_factors_dict), incidents)

@st.cache_data(show_spinner=False)
def cache_forecast(_engine: PredictiveAnalyticsEngine, kpi_df: pd.DataFrame) -> pd.DataFrame:
    return _engine.generate_forecast(kpi_df)

@st.cache_data(show_spinner=False)
def cache_allocations(_engine: PredictiveAnalyticsEngine, kpi_df: pd.DataFrame) -> dict:
    return _engine.generate_allocation_recommendations(kpi_df)
...
class Dashboard:
    ...
    def __init__(self, dm: DataManager, engine: PredictiveAnalyticsEngine):
        ...
        if 'env_factors' not in st.session_state:
            default_pop_density = self.config.get('defaults', {}).get('population_density', 50000)
            st.session_state['env_factors'] = EnvFactors(
                is_holiday=False, weather="Clear", traffic_level=1.0, major_event=False,
                population_density=self.dm.zones_gdf['population'].mean() if not self.dm.zones_gdf.empty else default_pop_density,
                air_quality_index=self.config.get('defaults', {}).get('air_quality_index', 50.0),
                heatwave_alert=False, day_type='Weekday', time_of_day='Midday',
                public_event_type='None', hospital_divert_status=0.0, police_activity='Normal',
    ...
    def render(self):
        ...
        with st.spinner("Executing Advanced Analytics Pipeline..."):
            env_dict = asdict(env_factors)  # Convert to dict for caching
            current_incidents = cache_current_incidents(self.dm, env_dict)
            kpi_df = cache_kpis(self.engine, historical_data, env_dict, current_incidents)
            forecast_df = cache_forecast(self.engine, kpi_df)
            allocations = cache_allocations(self.engine, kpi_df)
    ...
    def _render_sidebar(self):
        ...
        with st.sidebar.expander("General Environmental Factors", expanded=True):
            is_holiday = st.checkbox("Is Holiday", value=env.is_holiday, key="is_holiday_sb", help="Toggle if today is a public holiday.")
            weather = st.selectbox("Weather", ["Clear", "Rain", "Fog"], index=["Clear", "Rain", "Fog"].index(env.weather), key="weather_sb", help="Select current weather conditions.")
            aqi = st.slider("Air Quality Index (AQI)", 0.0, 500.0, env.air_quality_index, 5.0, key="aqi_sb", help="Air Quality Index (0-500). Higher values indicate worse air quality.")
            heatwave = st.checkbox("Heatwave Alert", value=env.heatwave_alert, key="heatwave_sb", help="Toggle if a heatwave alert is active.")

        with st.sidebar.expander("Contextual & Event-Based Factors", expanded=True):
            day_type = st.selectbox("Day Type", ['Weekday', 'Friday', 'Weekend'], index=['Weekday', 'Friday', 'Weekend'].index(env.day_type), help="Select the type of day.")
            time_of_day = st.selectbox("Time of Day", ['Morning Rush', 'Midday', 'Evening Rush', 'Night'], index=['Morning Rush', 'Midday', 'Evening Rush', 'Night'].index(env.time_of_day), help="Select the time of day.")
            public_event_type = st.selectbox("Public Event Type", ['None', 'Sporting Event', 'Concert/Festival', 'Public Protest'], index=['None', 'Sporting Event', 'Concert/Festival', 'Public Protest'].index(env.public_event_type), help="Select the type of public event, if any.")
            school_in_session = st.checkbox("School In Session", value=env.school_in_session, help="Toggle if schools are in session.")

        with st.sidebar.expander("System Strain & Response Factors", expanded=True):
            traffic = st.slider("General Traffic Level", 0.5, 3.0, env.traffic_level, 0.1, key="traffic_sb", help="A multiplier for traffic congestion (0.5-3.0).")
            hospital_divert_status = st.slider("Hospital Divert Status (%)", 0, 100, int(env.hospital_divert_status * 100), 5, help="Percentage of hospitals on diversion (0-100%).") / 100.0
            police_activity = st.selectbox("Police Activity Level", ['Low', 'Normal', 'High'], index=['Low', 'Normal', 'High'].index(env.police_activity), help="Select the level of police activity.")
        ...
        # Explicit comparison to avoid reliance on __eq__
        env_changed = any([
            new_env.is_holiday != env.is_holiday,
            new_env.weather != env.weather,
            new_env.traffic_level != env.traffic_level,
            new_env.major_event != env.major_event,
            new_env.air_quality_index != env.air_quality_index,
            new_env.heatwave_alert != env.heatwave_alert,
            new_env.day_type != env.day_type,
            new_env.time_of_day != env.time_of_day,
            new_env.public_event_type != env.public_event_type,
            new_env.hospital_divert_status != env.hospital_divert_status,
            new_env.police_activity != env.police_activity,
            new_env.school_in_session != env.school_in_session
        ])

        if st.sidebar.button("Apply Changes", use_container_width=True):
            if env_changed:
        ...
        uploaded_file = st.sidebar.file_uploader("Upload Historical Incidents (JSON)", type=["json"], help="Upload a JSON file containing historical incident data.")
        if uploaded_file:
            try:
                ...
                st.sidebar.error(f"Error loading JSON data: {e}. Ensure the file is a valid JSON format.")
        ...
        with st.spinner("Generating Report..."):
            try:
                ...
                if pdf_buffer.getvalue():
                    ...
                    st.sidebar.error("Failed to generate PDF report: Empty output.")
                    st.sidebar.info("Ensure KPI data and forecast data are available. Check logs for details.")
            except Exception as e:
                logger.error(f"Report generation failed: {e}", exc_info=True)
                st.sidebar.error(f"Failed to generate PDF report: {e}")
                st.sidebar.info("Check logs for detailed error information.")

    def _render_main_dashboard_tab(self, kpi_df: pd.DataFrame, allocations: dict, incidents: list):
        ...
        c2.metric("Available Ambulances", sum(1 for a in self.dm.ambulances.values() if a['status'] == 'Disponible'), delta_color="off", help="Number of ambulances available for dispatch.")
        c3.metric("Highest Zone Risk", f"{kpi_df['Integrated_Risk_Score'].max():.3f}" if not kpi_df.empty else "N/A", help="The peak Integrated Risk Score across all zones.")
        c4.metric("System Adequacy", f"{kpi_df['Resource Adequacy Index'].mean():.1%}" if not kpi_df.empty else "N/A", help="Ratio of available units to system-wide expected incidents, penalized by hospital strain.")
        ...
        with col2:
            ...
            alloc_df = pd.DataFrame(list(allocations.items()), columns=['Zone', 'Recommended Units']).set_index('Zone') if allocations else pd.DataFrame(columns=['Zone', 'Recommended Units'])
            ...
            if not kpi_df.empty:
                ...
                st.info("No risk data available to display top zones.")

    def _render_kpi_deep_dive_tab(self, kpi_df: pd.DataFrame, forecast_df: pd.DataFrame):
        ...
        if not kpi_df.empty:
            ...
            st.info("Insufficient data to generate advanced analytical plots. Upload historical data or adjust environmental factors.")
        ...
        if not forecast_df.empty:
            ...
            st.dataframe(forecast_pivot.style.format("{:.3f}").background_gradient(cmap='magma', axis=1), use_container_width=True)
        else:
            st.info("No forecast data available to display.")

    def _render_map(self, kpi_df: pd.DataFrame):
        if self.dm.zones_gdf.empty or kpi_df.empty:
            st.info("Map data unavailable. Displaying zone risk table instead.")
            if not kpi_df.empty:
                st.dataframe(kpi_df[['Zone', 'Integrated_Risk_Score']].set_index('Zone').style.format("{:.3f}"), use_container_width=True)
            return

        try:
            # Validate GeoDataFrame
            if 'name' not in self.dm.zones_gdf.columns or self.dm.zones_gdf.geometry.is_empty.all():
                raise ValueError("Invalid GeoDataFrame: Missing 'name' column or empty geometries.")
            
            map_gdf = self.dm.zones_gdf.join(kpi_df.set_index('Zone'))
            map_gdf.reset_index(inplace=True)
            center = map_gdf.unary_union.centroid
            zoom_start = self.config.get('map', {}).get('zoom_start', 12)
            m = folium.Map(location=[center.y, center.x], zoom_start=zoom_start, tiles="cartodbpositron", prefer_canvas=True)
            ...
            choropleth = folium.Choropleth(
                ...
                fill_color='viridis', fill_opacity=0.7, line_opacity=0.2,
                legend_name='Integrated Risk Score'
            ).add_to(m)
            ...
            folium.GeoJson(
                map_gdf, style_function=lambda x: {'color': 'black', 'weight': 1, 'fillOpacity': 0},
            ...
            st.error(f"Error rendering map: {e}. Displaying zone risk table instead.")
            if not kpi_df.empty:
                st.dataframe(kpi_df[['Zone', 'Integrated_Risk_Score']].set_index('Zone').style.format("{:.3f}"), use_container_width=True)
    ...
    def _plot_risk_contribution_sunburst(self, kpi_df: pd.DataFrame) -> Tuple[str, str, go.Figure]:
        ...
        if kpi_df.empty:
            return title, help_text, go.Figure()
        ...
        fig.update_layout(margin=dict(t=40, l=10, r=10, b=10), title_text=f"Risk Breakdown for Zone: {zone_name}", title_x=0.5, height=400)
    ...
    def _plot_advanced_model_correlation(self, kpi_df: pd.DataFrame) -> Tuple[str, str, go.Figure]:
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorscale='viridis', text=corr_df.values, texttemplate="%{text:.2f}", hoverongaps=False
        ))
        fig.update_layout(margin=dict(t=40, l=10, r=10, b=10), height=400, title_text="Correlation Matrix of Advanced KPIs", title_x=0.5)
        return title, help_text, fig
    ...
    def _plot_vulnerability_quadrant(self, kpi_df: pd.DataFrame) -> Tuple[str, str, go.Figure]:
        ...
        fig = px.scatter(
            ...
            hover_name="Zone", color_continuous_scale="viridis", size_max=18
        )
