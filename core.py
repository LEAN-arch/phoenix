...
class PredictiveAnalyticsEngine:
    ...
    def __init__(self, dm: DataManager, config: Dict[str, Any]):
        ...
        if total_weight > 0:
            self.method_weights = {k: v / total_weight for k, v in weights_config.items()}
        else:
            self.method_weights = {}

        # Pre-calculate static GNN risk for efficiency
    ...
    def generate_kpis(_self, historical_data: List[Dict], env_factors: EnvFactors, current_incidents: List[Dict]) -> pd.DataFrame:
        kpi_cols = [
            ...
            'Bayesian Confidence Score', 'Information Value Index', 'Ensemble Risk Score',
        ...
        incidents_with_zones = gpd.sjoin(incident_gdf, _self.dm.zones_gdf, how="inner", predicate="within")
        incidents_with_zones.rename(columns={'index_right': 'Zone'}, inplace=True)
        if incidents_with_zones.empty: return kpi_df.reset_index().rename(columns={'index': 'Zone'})

        # --- Base KPI Calculations ---
        self._calculate_base_kpis(kpi_df, incidents_with_zones, historical_data, env_factors)

        # --- Advanced Analytics Layer ---
        self._calculate_advanced_kpis(kpi_df, incidents_with_zones)
    ...
    def _calculate_base_kpis(self, kpi_df: pd.DataFrame, incidents: gpd.GeoDataFrame, history: List[Dict], env: EnvFactors):
        ...
        violence_counts = incidents[incidents['type'] == 'Trauma-Violence']['Zone'].value_counts().reindex(self.dm.zones, fill_value=0)
        accident_counts = incidents[incidents['type'] == 'Trauma-Accident']['Zone'].value_counts().reindex(self.dm.zones, fill_value=0)
        medical_counts = incidents[incidents['type'].isin(['Medical-Chronic', 'Medical-Acute'])]['Zone'].value_counts().reindex(self.dm.zones, fill_value=0)
        
        day_time_multiplier = {'Weekday': 1.0, 'Friday': 1.2, 'Weekend': 1.3}.get(env.day_type, 1.0)
        day_time_multiplier *= {'Morning Rush': 1.1, 'Midday': 0.9, 'Evening Rush': 1.2, 'Night': 1.4}.get(env.time_of_day, 1.0)
        event_multiplier = 1.0; violence_event_mod = 1.0; medical_event_mod = 1.0
        if env.public_event_type != 'None':
            event_multiplier = {'Sporting Event': 1.6, 'Concert/Festival': 1.8, 'Public Protest': 2.0}.get(env.public_event_type, 1.0)
            violence_event_mod = {'Sporting Event': 1.8, 'Public Protest': 2.5}.get(env.public_event_type, 1.0)
            medical_event_mod = {'Concert/Festival': 2.0}.get(env.public_event_type, 1.0)
        effective_traffic = env.traffic_level * (1.0 if env.school_in_session else 0.8)
        police_activity_mod = {'Low': 1.1, 'Normal': 1.0, 'High': 0.85}.get(env.police_activity, 1.0)
        ...
        hawkes = self.model_params['hawkes_process']; sir = self.model_params['sir_model']
        ...
        kpi_df['Medical Surge Score'] = (self.dm.zones_gdf['population'].apply(lambda s: sir['beta']*medical_counts.get(s,0)/(s+1e-9)-sir['gamma'])*medical_event_mod).clip(0,1)
    ...
    def _calculate_advanced_kpis(self, kpi_df: pd.DataFrame, incidents_with_zones: gpd.GeoDataFrame):
        """Calculates and integrates advanced KPIs."""
        kpi_df['Ensemble Risk Score'] = self._calculate_ensemble_risk_score(kpi_df)
        ...
        kpi_df['GNN_Structural_Risk'] = self.gnn_structural_risk # Use pre-calculated static risk
    ...
    def _calculate_ensemble_risk_score(self, kpi_df: pd.DataFrame) -> pd.Series:
        if kpi_df.empty or not self.method_weights: return pd.Series(0.0, index=kpi_df.index)
        # ... Ensemble calculation logic is unchanged ...
        return pd.Series(0.5, index=kpi_df.index) # Abridged for clarity

    def generate_forecast(self, kpi_df: pd.DataFrame) -> pd.DataFrame:
        if kpi_df.empty: return pd.DataFrame()
        # ... Forecast logic is unchanged, now uses Integrated_Risk_Score ...
        return pd.DataFrame() # Abridged for clarity
    ...
    def generate_allocation_recommendations(self, kpi_df: pd.DataFrame) -> Dict[str, int]:
        ...
        # ... Allocation logic is unchanged, now uses Integrated_Risk_Score ...
        return {} # Abridged for clarity
