...
class DataManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config; self.data_config = config['data']; self.zones = list(self.data_config['zones'].keys())
        self.zones_gdf = self._build_zones_gdf(); self.road_graph = self._build_road_graph(); self.ambulances = self._initialize_ambulances()
        try:
    ...
    def _build_road_graph(self) -> nx.Graph:
        G = nx.Graph(); G.add_nodes_from(self.zones); edges = self.data_config.get('road_network', {}).get('edges', [])
        for u, v, weight in edges:
            if u in G.nodes and v in G.nodes and isinstance(weight, (int, float)) and weight > 0: G.add_edge(u, v, weight=float(weight))
        return G

    def _build_zones_gdf(self) -> gpd.GeoDataFrame:
        zone_data = []
        for name, data in self.data_config['zones'].items():
            poly = Polygon([(lon, lat) for lat, lon in data['polygon']]); poly = poly.buffer(0) if not poly.is_valid else poly
    ...
    def get_current_incidents(self, env_factors: EnvFactors) -> List[Dict[str, Any]]:
        api_config = self.data_config.get('real_time_api', {}); endpoint = api_config.get('endpoint', '')
        try:
    ...
    def _validate_incidents(self, incidents: List[Dict]) -> List[Dict]:
        ...
        for inc in incidents:
            ...
            if all(k in inc for k in ['id', 'type', 'triage']) and isinstance(loc, dict) and 'lat' in loc and 'lon' in loc:
                try: inc['location']['lat'] = float(loc['lat']); inc['location']['lon'] = float(loc['lon']); valid_incidents.append(inc)
                except (ValueError, TypeError): logger.warning(f"Skipping incident {inc.get('id', 'N/A')} due to invalid location data.")
    ...
    def generate_sample_history_file(self) -> io.BytesIO:
        ...
        buffer = io.BytesIO(); buffer.write(json.dumps([{'incidents': self._generate_synthetic_incidents(default_env), 'timestamp': (datetime.utcnow() - timedelta(hours=i*24)).isoformat()} for i in range(3)], indent=2).encode('utf-8')); buffer.seek(0)
        return buffer
...
class PredictiveAnalyticsEngine:
    ...
    def __init__(self, dm: DataManager, config: Dict[str, Any]):
        self.dm = dm; self.config = config; self.model_params = config['model_params']
        self.bn_model = self._build_bayesian_network(); self.tcnn_model = TCNN(**config['tcnn_params']) if TORCH_AVAILABLE else None
        self.forecast_df = pd.DataFrame()
        weights_config = self.model_params.get('ensemble_weights', {}); total_weight = sum(weights_config.values())
        if total_weight > 0: self.method_weights = {k: v / total_weight for k, v in weights_config.items()}
        else: self.method_weights = {}
    ...
    def _build_bayesian_network(_self) -> Optional[BayesianNetwork]:
        ...
        try:
            bn_config = _self.config['bayesian_network']; model = BayesianNetwork(bn_config['structure'])
    ...
    def _post_process_allocations(self, allocations: Dict[str, Any], available_units: int, zones: List[str], sort_key: pd.Series) -> Dict[str, int]:
        """Ensures the final integer sum of allocations equals available_units after rounding."""
    ...
    def _allocate_proportional(self, kpi_df: pd.DataFrame, available_units: int) -> Dict[str, int]:
        """Basic proportional allocation based on risk scores."""
    ...
    def _allocate_milp(self, kpi_df: pd.DataFrame, available_units: int) -> Dict[str, int]:
        """Optimizes allocation using Mixed-Integer Linear Programming to maximize risk coverage."""
    ...
    def _allocate_nlp(self, kpi_df: pd.DataFrame, available_units: int) -> Dict[str, int]:
        """Optimizes allocation using Non-Linear Programming for more realistic modeling."""
    ...
    def generate_allocation_recommendations(self, kpi_df: pd.DataFrame) -> Dict[str, int]:
        """
        Generates ambulance allocation recommendations using a configurable strategy.
        This method acts as a dispatcher to the underlying OR models.
        """
